...
def test_return_user(test_user, override_get_current_user):
...
def test_change_password_success(test_user, override_get_current_user):
    response = client.put("/user/password", json={"password": "testpassword",
                                                "new_password": "newpassword"})
    assert response.status_code == status.HTTP_204_NO_CONTENT


def test_change_password_invalid_current_password(test_user, override_get_current_user):
